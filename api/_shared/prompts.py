...
# ---------------------------------------------------------------------------
# Per-section regeneration prompts
# ---------------------------------------------------------------------------

_REGEN_STYLES_FORMAT = """\
## Output Format
You MUST output ONLY the style prompt section with this delimiter:
...
CRITICAL: Output ONLY the ---STYLES--- section. No analysis, no lyrics, no exclusions, \
no conversational filler. Start immediately with ---STYLES---."""

PROMPT_REGEN_STYLES = f"""{_BASE_PERSONA}

You will receive the current lyrics and analysis for an existing song. Your task is to generate \
a FRESH, ALTERNATIVE style prompt that matches the song's sonic identity but explores a different \
creative angle — different instrumentation choices, production techniques, or genre inflection \
while staying true to the song's emotional core.

Do NOT simply repeat the existing style prompt. Offer a genuinely different but equally valid \
sonic interpretation of the same lyrics.

{_STYLE_INSTRUCTIONS}

{_REGEN_STYLES_FORMAT}"""

_REGEN_EXCLUDE_FORMAT = """\
## Output Format
You MUST output ONLY the exclusion section with this delimiter:
...
CRITICAL: Output ONLY the ---EXCLUDE--- section. No analysis, no lyrics, no styles, \
no conversational filler. Start immediately with ---EXCLUDE---."""

PROMPT_REGEN_EXCLUDE = f"""{_BASE_PERSONA}

You will receive the current style prompt and lyrics for an existing song. Your task is to generate \
a FRESH set of exclusion terms that protect this track's sonic identity. Consider what would \
actively harm THIS specific production and exclude it.

{_EXCLUDE_INSTRUCTIONS}

{_REGEN_EXCLUDE_FORMAT}"""

_REGEN_LYRICS_FORMAT = """\
## Output Format
You MUST output ONLY the lyrics section with this delimiter:
...
CRITICAL: Output ONLY the ---LYRICS--- section. No analysis, no styles, no exclusions, \
no conversational filler. Start immediately with ---LYRICS---."""

PROMPT_REGEN_LYRICS = f"""{_BASE_PERSONA}

You will receive the current style prompt, exclude styles, and analysis for an existing song. \
Your task is to compose ENTIRELY NEW lyrics that match the same sonic blueprint and emotional \
intent, but with fresh imagery, new metaphors, and different narrative choices. The new lyrics \
should feel like an alternate version of the same song — same vibe, different words.

Preserve the same structural approach (verse/chorus/bridge flow) but create completely original \
lyric content. Match the energy arc described in the analysis.

{_LYRICS_INSTRUCTIONS}

{_REGEN_LYRICS_FORMAT}"""

_REGEN_ANALYSIS_FORMAT = """\
## Output Format
You MUST output ONLY the analysis section with this delimiter:
...
CRITICAL: Output ONLY the ---ANALYSIS--- section. No styles, no lyrics, no exclusions, \
no conversational filler. Start immediately with ---ANALYSIS---."""

PROMPT_REGEN_ANALYSIS = f"""{_BASE_PERSONA}

You will receive the current lyrics and style prompt for an existing song. Your task is to perform \
a FRESH, deep analysis of the song, potentially uncovering new insights about its sonic DNA, \
phonetic patterns, and semantic weight that a previous analysis may have missed.

Look deeper — find subtleties in the phonetic textures, identify secondary emotional currents, \
and map narrative peaks with fresh perspective.
...
PROMPTS = {
    ...
    "regen_styles": PROMPT_REGEN_STYLES,
    "regen_exclude": PROMPT_REGEN_EXCLUDE,
    "regen_lyrics": PROMPT_REGEN_LYRICS,
    "regen_analysis": PROMPT_REGEN_ANALYSIS,
}
