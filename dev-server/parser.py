...
def parse_full_response(raw: str) -> dict:
    ...
    # Log missing sections for debugging (but don't fail — use graceful defaults)
    expected = {"analysis", "styles", "exclude", "lyrics"}
    missing = expected - set(sections.keys())
    if missing:
        import logging
        logging.getLogger("parser").warning(f"Missing sections in LLM response: {missing}")
...
def parse_single_section(raw: str, section: str) -> dict:
    """Parse a single-section regeneration response and return a partial result dict."""
    sections = _split_sections(raw)

    if section == "styles":
        return {"styles": sections.get("styles", raw.strip())}
    elif section == "exclude_styles":
        return {"exclude_styles": sections.get("exclude", raw.strip())}
    elif section == "lyrics":
        lyrics = sections.get("lyrics", raw.strip())
        return {"lyrics": lyrics, "plain_lyrics": _strip_to_plain(lyrics)}
    elif section == "analysis":
        analysis_raw = sections.get("analysis", raw.strip())
        analysis = _parse_analysis(analysis_raw)
